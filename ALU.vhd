...
-- It will work with BIT_AMT number of bits.
...
architecture behaviour of ALU is
	...
	signal z_buffer : std_logic_vector(BIT_AMT downto 0);
	
begin
	
	process(op, a, b, carry_in)
	
		variable carry_in_val : signed(BIT_AMT downto 0);
	
	begin	
	
		if carry_in = '1' then
			carry_in_val := to_signed(1, BIT_AMT+1);
		elsif carry_in = '0' then
			carry_in_val := to_signed(0, BIT_AMT+1);
		end if;
		
		case(op) is
		when INC_A =>
			z_buffer <= std_logic_vector(resize(signed(a), BIT_AMT+1) + 1);
		when DEC_A =>
			z_buffer <= std_logic_vector(resize(signed(a), BIT_AMT+1) - 1);
		when ADD_A_B =>
			z_buffer <= std_logic_vector(
								resize(signed(a), BIT_AMT+1) + 
								resize(signed(b), BIT_AMT+1)
							);
		when ADC_A_B =>
			z_buffer <= std_logic_vector(
								resize(signed(a), BIT_AMT+1) + 
		...
		when PASS_A =>
			z_buffer <= std_logic_vector(resize(signed(a), BIT_AMT+1));
		when PASS_B =>
			z_buffer <= std_logic_vector(resize(signed(b), BIT_AMT+1));
		when AND_A_B =>
			z_buffer <= std_logic_vector(
								resize(signed(a), BIT_AMT+1) AND
								resize(signed(b), BIT_AMT+1)
							);
		when OR_A_B =>
			z_buffer <= std_logic_vector(
								resize(signed(a), BIT_AMT+1) OR
								resize(signed(b), BIT_AMT+1)
							);
		when XOR_A_B =>
			z_buffer <= std_logic_vector(
								resize(signed(a), BIT_AMT+1) XOR
								resize(signed(b), BIT_AMT+1)
							);
		when NOT_A =>
			z_buffer <= std_logic_vector(resize(signed(NOT a), BIT_AMT+1));
		when SHL_A =>
			z_buffer <= std_logic_vector(resize(shift_left(signed(a), 1), BIT_AMT+1));
		when ROL_A =>
			z_buffer <= '0' & (a(BIT_AMT-2 downto 0) & a(BIT_AMT-1));
		when SHR_A =>
			z_buffer <= std_logic_vector(resize(shift_right(unsigned(a), 1), BIT_AMT+1));
		when ROR_A =>
			z_buffer <= '0' & (a(0) & a(BIT_AMT-1 downto 1));
		when others =>
			z_buffer <= (others => 'U');
		end case;
	end process;
	
	z <= z_buffer(BIT_AMT - 1 downto 0);
	
	carry_out <= z_buffer(BIT_AMT);
